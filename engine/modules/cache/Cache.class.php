...
class ModuleCache extends Module
{
    ...
    public function Init()
    {
        ...
        if ($this->sCacheType == SYS_CACHE_TYPE_FILE) {
            ...
            $this->oBackendCache = new Dklab_Cache_Backend_Profiler($oCache, array($this, 'CalcStats'));
            ...
            $this->oBackendCache = new Dklab_Cache_Backend_TagEmuWrapper(new Dklab_Cache_Backend_Profiler($oCache, array($this, 'CalcStats')));

        } elseif ($this->sCacheType == "SYS_CACHE_TYPE_REDIS") {
            $aConfigRedis = Config::Get('redis');

            $oCache = new Zend_Cache_Backend_Redis($aConfigRedis);
    ...
    public function SmartGet($sName)
    {
        ...
        if (($data = $this->Get($sName)) === false) {
            $this->Set($this->Get($this->sPrefixSmartCache . $sName), $sName, array(), 60); // храним данные из временного в основном недолго
        }
        return $data;
    }
