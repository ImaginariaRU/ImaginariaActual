...
<article class="topic topic-type-{$oTopic->getType()} js-topic">
	<header class="topic-header">
		...
		<a href="{$oBlog->getUrlFull()}" class="blog-url">{$oBlog->getTitle()|escape:'html'}</a> <small>&rarr;</small>
		...
		<div class="topic-info">
			...
			<time datetime="{date_format date=$oTopic->getDateAdd() format='c'}" title="{date_format date=$oTopic->getDateAdd() format='j F Y, H:i'}">
				<i class="icon icon-time"></i> {date_format date=$oTopic->getDateAdd() format="j F Y, H:i"}
			</time>
