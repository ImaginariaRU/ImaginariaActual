...
class PluginShowvotes_ModuleVote extends Module
{
    ...
    public function GetTopicVoters($oTopic)
    {
        ...
        //@todo: используется неоптимальный принцип - делается три запроса подряд с разными WHERE vote_direction
    ...
    public function IsAllowed()
    {
        ...
        switch (Config::Get('plugin.showvotes.can_see')) {
            case 'admin':
                ...
                break;
            case 'author':
                if (!$oUserCurrent || (!$oUserCurrent->isAdministrator() && $oUserCurrent->getId() != $oTopic->getUserId())) {
                    $bOk = false;
                }
                break;
            case 'user':
