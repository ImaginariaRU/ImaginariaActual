...
class PluginAceadminpanel_ModuleAdmin_MapperAdmin extends Mapper
{
    ...
    public function GetUserList(&$iCount, $iCurrPage, $iPerPage, $aFilter = Array(), $aSort = Array())
    {
        ...
        $sql =
            "
            SELECT u.user_id, user_login, user_date_register, user_skill, user_mail,
            ...
            TIMESTAMPDIFF(MINUTE, user_date_register, user_date_activate) AS activation_diff, count_topics, count_comments, count_votes
           
                FROM      " . Config::Get('db.table.user') . " AS u
                ...
                LEFT JOIN ( SELECT user_id, COUNT(*) AS count_topics FROM   " . Config::Get('db.table.topic')    . " GROUP BY user_id ) AS sub_topics   ON sub_topics.user_id   = u.user_id
                LEFT JOIN ( SELECT user_id, COUNT(*) AS count_comments FROM " . Config::Get('db.table.comment')  . " GROUP BY user_id ) AS sub_comments ON sub_comments.user_id = u.user_id
                LEFT JOIN ( SELECT user_voter_id, COUNT(*) AS count_votes FROM ls_vote lv GROUP BY user_voter_id ) AS count_votes ON count_votes.user_voter_id = u.user_id
        ...
        if ($aRows) {
            foreach ($aRows as $aRow) {
                ...
                $aReturn[] = Engine::GetEntity('User', $aRow); // ModuleUser_EntityUser
            }
        }
        return $aReturn;
    }
