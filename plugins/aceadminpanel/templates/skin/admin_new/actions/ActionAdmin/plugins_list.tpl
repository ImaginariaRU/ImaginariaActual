...
{/literal}
    ...
    <ul class="nav nav-tabs">
        ...
        <li {if $sMode=='all' || $sMode==''}class="active"{/if}><a
                    href="{router page='admin'}plugins/list/all/">{$oLang->_adm_all_plugins}</a></li>
        <li {if $sMode=='active'}class="active"{/if}><a
                    href="{router page='admin'}plugins/list/active/">{$oLang->_adm_active_plugins}</a></li>
        <li {if $sMode=='inactive'}class="active"{/if}><a
                    href="{router page='admin'}plugins/list/inactive/">{$oLang->_adm_inactive_plugins}</a></li>
    </ul>
            ...
            {foreach from=$aPluginList item=oPlugin}
                <tr id="{$oPlugin->GetId()}_row" class="{if $oPlugin->IsActive()}active{else}inactive{/if} selectable">
                    ...
                    <td class="action {if $oPlugin->IsActive()}deactivate{else}activate{/if}">
                        {if $oPlugin->isActive()}
                            <div class="btn-group btn-switch-on" title="{$oLang->adm_act_deactivate}"
                                 onclick="aceAdmin.plugin.turnOff('{$oPlugin->GetId()}'); return false;">
                                <button class="btn btn-outset"></button>
                                <button class="btn btn-inset">ON</button>
                            </div>
                        {else}
                            <div class="btn-group btn-switch-off" title="{$oLang->adm_act_activate}"
                                 onclick="aceAdmin.plugin.turnOn('{$oPlugin->GetId()}'); return false;">
