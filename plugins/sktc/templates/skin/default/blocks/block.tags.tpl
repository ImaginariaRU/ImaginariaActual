            ...
            {if $aTags}
                <ul class="tag-cloud word-wrap">
                    {foreach from=$aTags item=oTag}
                        ...
                        <li{if $sAction=='tag' and $sEvent==$oTag->getText()} class="active"{/if}><a
                                    href="{router page='tag'}{$oTag->getText()|escape:'url'}/"><span
                                        class="sktc-tag-text">{$oTag->getText()|escape:'html'}</span> <span
                                        class="sktc-tag-count">{$oTag->getCount()}</span></a></li>
                    {/foreach}
                </ul>
            {else}
                ...
                {if $aTagsUser}
                    <ul class="tag-cloud word-wrap">
                        {foreach from=$aTagsUser item=oTag}
                            <li><a href="{router page='tag'}{$oTag->getText()|escape:'url'}/"><span
                                            class="sktc-tag-text">{$oTag->getText()|escape:'html'}</span> <span
                                            class="sktc-tag-count">{$oTag->getCount()}</span></a></li>
                        {/foreach}
                    </ul>
