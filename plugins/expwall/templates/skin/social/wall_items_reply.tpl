{foreach from=$aReplyWall item=oReplyWall}
    ...
    <div id="wall-reply-item-{$oReplyWall->getId()}" class="js-wall-reply-item wall-item-reply">
        <a href="{$oReplyUser->getUserWebPath()}"><img src="{$oReplyUser->getProfileAvatarPath(48)}" alt="avatar"
                                                       class="avatar"/></a>

        <a href="{$oReplyUser->getUserWebPath()}"
           class="author">{if $oReplyUser->getProfileName()}{$oReplyUser->getProfileName()}{else}{$oReplyUser->getLogin()}{/if}</a>
        ·
