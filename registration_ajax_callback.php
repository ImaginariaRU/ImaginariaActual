...
$captcha_config = [
    'secret_key'    =>  '0x4AAAAAAAcFuwELQ6CEykG91oF07uMnwLQ',
    'site_key'      =>  '0x4AAAAAAAcFu1JCgXcTOghj'
];
...
function check_captcha_google($sCaptchaHash) {
    $data = array(
...
/**
 * https://packagist.org/packages/fluxsoft/php-turnstile
 *
 * @param $sCaptchaHash
 * @return true|void
 */
function check_captcha_turnstile()
{
    global $captcha_config;

    $turnstile = new Turnstile($captcha_config['secret_key']);
...
if ($post_data['stat'] == 'add') {
    /*if (!check_captcha_google($post_data['captcha'])) {
        $responseCode = 'err_cap';
        return;
    }*/

    if (!check_captcha_turnstile()) {
        $responseCode = 'err_cap';
        return;
    }

    if (!checkParam($post_data['login'])) {
        $responseCode = 'err_login';
        return;
    }
    if (!checkParam($post_data['email'])) {
        $responseCode = 'err_email';
        return;
    }
    if (!checkParam($post_data['pass'])) {
        $responseCode = 'err_pass';
        return;
    }

    $responseCode = create_user($post_data['login'], $post_data['email'], $post_data['pass']);
